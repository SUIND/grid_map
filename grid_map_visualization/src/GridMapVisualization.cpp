    ...
    : nodeHandle_(nodeHandle), visualizationsParameter_(parameterName), factory_(nodeHandle_), isSubscribed_(false)
{
  ...
  readParameters();
  activityCheckTimer_ =
      nodeHandle_.createTimer(activityCheckDuration_, &GridMapVisualization::updateSubscriptionCallback, this);
  initialize();
}
...
bool GridMapVisualization::readParameters()
{
  ...
  for (unsigned int i = 0; i < config.size(); ++i)
  {
    ...
    {
      ROS_ERROR("%s: Could not add a visualization because no type was given", visualizationsParameter_.c_str());
      return false;
    }
    else if (!config[i].hasMember("name"))
    {
      ROS_ERROR("%s: Could not add a visualization because no name was given", visualizationsParameter_.c_str());
      return false;
    }
    else
    {
      ...
      for (int j = i + 1; j < config.size(); ++j)
      {
        ...
        if (!config[j].hasMember("name") || config[i]["name"].getType() != XmlRpc::XmlRpcValue::TypeString ||
        ...
        if (namei == namej)
        {
          ROS_ERROR("%s: A visualization with the name '%s' already exists.", visualizationsParameter_.c_str(),
                    namei.c_str());
          return false;
        }
      }
  ...
  for (int i = 0; i < config.size(); ++i)
  {
    ...
    ROS_INFO("%s: Configured visualization of type '%s' with name '%s'.", visualizationsParameter_.c_str(),
