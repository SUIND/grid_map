                                               ...
                                               const std::string& heightLayerName, const std::string& newFrameId)
{
  ...
  if (std::abs(yawPitchRoll.y()) >= 3 && std::abs(yawPitchRoll.z()) >= 3)
  ...
  for (const auto& layer : gridMapSource.getLayers())
  {
    ...
    if (firstLayer)
    {
      ...
      // https://docs.opencv.org/3.4/da/d54/group__imgproc__transform.html#gafbbc470ce83812914a70abfb604f4326 and
