...
TEST(ImageConversion, roundTrip8UC3)
{
  ...
  replaceNan(mapIn.get("layer"), minValue);  // When we move `mapIn`, new areas are filled with NaN. As `toImage` does
                                             // not support NaN, we replace NaN with `minValue` instead.
