...
class GridMapCvProcessing
{
 public:
   ...
   * the original map.
   * Also note that the map treats the height layer differently from other layers. I.e, the translation in z-direction
   * is applied only to the height layer. For layers other than the height layer that contain geometric information this
   * function might compute unexpected results. E.g transforming the layers normals_x/y/z will only represent the same
   * values at the transformed location but will not transform the normal vectors into the new coordinate system.
   *
