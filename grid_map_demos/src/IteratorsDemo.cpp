...
{
IteratorsDemo::IteratorsDemo(ros::NodeHandle& nodeHandle) : nodeHandle_(nodeHandle), map_(vector<string>({"type"}))
{
...
void IteratorsDemo::demoEllipseIterator()
{
  ...
  for (grid_map::EllipseIterator iterator(map_, center, length, M_PI_4); !iterator.isPastEnd(); ++iterator)
  {
    map_.at("type", *iterator) = 1.0;
    publish();
    ros::Duration duration(0.02);
...
void IteratorsDemo::demoSlidingWindowIterator()
{
  ...
  for (grid_map::SlidingWindowIterator iterator(map_, "copy", edgeHandling, windowSize); !iterator.isPastEnd();
       ++iterator)
  {
