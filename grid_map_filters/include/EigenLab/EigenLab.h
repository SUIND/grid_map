...
class Value
{
 ...
 public:
  ...
  Value(const typename Derived::Scalar s)
      : mLocal(Derived::Constant(1, 1, s)), mShared(mLocal.data(), mLocal.rows(), mLocal.cols()), mIsLocal(true)
  {
  }
  ...
  Value(const typename Derived::Scalar *data, size_t rows = 1, size_t cols = 1)
      : mShared(const_cast<typename Derived::Scalar *>(data), rows, cols), mIsLocal(false)
  {
  }
...
class Parser
{
 ...
 private:
  ...
  struct Chunk
  {
    ...
    Chunk(const std::string &str = "", int t = -1, const Value<Derived> &val = Value<Derived>())
        : field(str), type(t), value(val), row0(-1), col0(-1), rows(-1), cols(-1)
    {
    }
  };
  enum ChunkType
 ...
 private:
  ...
  std::string::const_iterator findClosingBracket(const std::string &str,
  ...
  bool evalFunction_2_lt(const std::string &name, Value<Derived> &arg0, Value<Derived> &arg1, Value<Derived> &result,
                         std::false_type);
  bool evalFunction_2_lt(const std::string &name, Value<Derived> &arg0, Value<Derived> &arg1, Value<Derived> &result,
                         std::true_type);
...
#ifdef DEBUG
  void test_w_lt(size_t &numFails, typename Derived::Scalar &s, Derived &a34, Derived &b34, Derived &c43, Derived &v,
                 std::true_type);
  void test_w_lt(size_t &numFails, typename Derived::Scalar &s, Derived &a34, Derived &b34, Derived &c43, Derived &v,
                 std::false_type);
  size_t test();
...
Parser<Derived>::Parser() : mOperators1("+-*/^()[]="), mOperators2(".+.-.*./.^"), mCacheChunkedExpressions(false)
{
...
Value<Derived> Parser<Derived>::eval(const std::string &expression)
{
  ...
  {
    if (!isVariable(chunks[0].field)) throw std::runtime_error("Unknown variable '" + chunks[0].field + "'.");
...
void Parser<Derived>::splitEquationIntoChunks(const std::string &expression, ChunkArray &chunks, std::string &code)
{
  ...
  for (std::string::const_iterator it = expression.begin(); it != expression.end();)
  {
    ...
    else if (ci == '(' && (prevType == VALUE || prevType == VARIABLE))
    {
      ...
      if (args.size() == 1)
      {
        if (cols == 1)
        {
          ...
          chunks.back().col0 = 0;
          chunks.back().rows =
              last + 1 - first;  //(last == -1 ? int(chunks.back().value.matrix().rows()) : last + 1) - first;
          chunks.back().cols = 1;
        }
        else if (rows == 1)
        {
          ...
          chunks.back().rows = 1;
          chunks.back().cols =
              last + 1 - first;  //(last == -1 ? int(chunks.back().value.matrix().cols()) : last + 1) - first;
        }
        else
        {
    ...
    else
    {
      ...
      {
        if (isOperator(*jt) || (jt + 1 != expression.end() && isOperator(std::string(jt, jt + 2)))) break;
      }
...
#endif
#endif
  if (cacheExpressions()) mCachedChunkedExpressions[expression] = chunks;
}

template <typename Derived>
std::string::const_iterator Parser<Derived>::findClosingBracket(const std::string &str,
                                                                const std::string::const_iterator openingBracket,
                                                                const char closingBracket) const
{
  int depth = 1;
...
{
  if (str.empty()) throw std::runtime_error("Empty index range.");
  ValueXi valuei;
  ParserXi parseri;
  size_t pos;
  for (std::string::const_iterator it = str.begin(); it != str.end(); it++)
  {
    if ((*it) == ':')
    {
      ...
      if (firstStr.empty() || lastStr.empty()) throw std::runtime_error("Missing indices for '" + str + "'.");
      ...
      if (valuei.matrix().size() != 1) throw std::runtime_error("Invalid indices '" + str + "'.");
  ...
  valuei = parseri.eval(firstStr);
  if (valuei.matrix().size() != 1) throw std::runtime_error("Invalid index '" + str + "'.");
  (*first) = valuei.matrix()(0, 0);
  (*last) = (*first);
}
...
void Parser<Derived>::evalMatrixExpression(const std::string &str, Value<Derived> &mat)
{
  ...
  for (size_t i = 0; i < rows.size(); i++)
  {
    ...
    for (size_t j = 0; j < cols.size(); j++)
    {
      ...
      if (pos != std::string::npos)
      {
        ...
        if (pos != std::string::npos)
        {
          ...
          else if ((slast - sfirst >= 0 && sstep > 0) || (slast - sfirst <= 0 && sstep < 0))
          {
            ...
            for (int k = 0; k < n; ++k) submatrix.local()(0, k) = sfirst + k * sstep;
            submatrix.mapLocal();
          }
          else
          {
        ...
        else
        {
          ...
          else if (slast - sfirst >= 0)
          {
            ...
            submatrix.local().resize(1, n);
            for (int k = 0; k < n; ++k) submatrix.local()(0, k) = sfirst + k;
            submatrix.mapLocal();
          }
          else
          {
      ...
      if (j > 0 && size_t(submatrix.matrix().cols()) != nrows)
        throw std::runtime_error("Invalid matrix definition '[" + str + "]'. Successive column entries '" +
                                 cols[int(j) - 1] + "' and '" + cols[j] + "' do not have the same number of rows.");
    ...
    if (row0 > 0 && ncols != temp[int(row0) - 1].size())
      throw std::runtime_error("Invalid matrix definition '[" + str + "]'. Successive row entries '" +
                               rows[int(i) - 1] + "' and '" + rows[i] + "' do not have the same number of columns.");
    row0 += nrows;
  }
  nrows = temp.size();
  if (nrows == 0) return;
  ...
  {
    for (size_t col = 0; col < ncols; col++) mat.matrix()(row, col) = temp[row][col];
  }
  mat.mapLocal();
}

template <typename Derived>
bool Parser<Derived>::evalFunction_1_lt(const std::string &name, Value<Derived> &arg, Value<Derived> &result,
                                        std::true_type)
{
  if (name == "min")
  {
...
template <typename Derived>
bool Parser<Derived>::evalFunction_1_lt(const std::string & /*name*/, Value<Derived> & /*arg0*/,
...
template <typename Derived>
bool Parser<Derived>::evalFunction_2_lt(const std::string &name, Value<Derived> &arg0, Value<Derived> &arg1,
                                        Value<Derived> &result, std::true_type)
{
  ...
  else if (name == "absmax")
  {
    ...
    if (dim == 0)
    {
      ...
      {
        if (std::abs(result.matrix()(i)) < std::abs(minimum(i))) result.matrix()(i) = minimum(i);
      }
      return true;
    }
    else if (dim == 1)
...
template <typename Derived>
bool Parser<Derived>::evalFunction_2_lt(const std::string & /*name*/, Value<Derived> & /*arg0*/,
...
{
  size_t pos = str.find(":");
  if (pos == std::string::npos) throw std::runtime_error("Invalid numeric range '" + str + "'.");
  ...
  if (pos2 == std::string::npos)
  {
    ...
    if (sfirst > slast) throw std::runtime_error("Invalid numeric range '" + str + "'. Must not reverse.");
    int n = 1 + floor(slast - sfirst);
    mat.local().resize(1, n);
    for (int i = 0; i < n; i++) mat.local()(0, i) = sfirst + i;
    mat.mapLocal();
  }
  else
  {
    ...
    else if (sfirst < slast && sstep > 0)
    {
      ...
      mat.local().resize(1, n);
      for (int i = 0; i < n; i++) mat.local()(0, i) = sfirst + i * sstep;
      mat.mapLocal();
    }
    else if (sfirst > slast && sstep < 0)
    {
...
void Parser<Derived>::evalIndices(ChunkArray &chunks)
{
  ...
  for (typename ChunkArray::iterator it = chunks.begin(); it != chunks.end(); it++)
  {
    if (it->row0 != -1 &&
        (it->type == VALUE ||
         (it->type == VARIABLE && (it + 1 == chunks.end() || (it + 1)->type != OPERATOR || (it + 1)->field != "="))))
    {
      if (it->type == VALUE)
      {
        ...
        (rhs->type == VALUE || rhs->type == VARIABLE))
    {
      ...
      else if (rhs->type == VARIABLE)
      {
        if (!isVariable(rhs->field))
          throw std::runtime_error("Attempted operation '" + op->field + rhs->field + "' on uninitialized variable '" +
                                   rhs->field + "'.");
    ...
    if ((op->type == OPERATOR) && (op->field == "^" || op->field == ".^"))
    {
      if (lhs->type == VARIABLE)
      {
        if (!isVariable(lhs->field))
          throw std::runtime_error("Attempted operation '" + lhs->field + op->field + rhs->field +
    ...
    if ((op->type == OPERATOR) && (op->field == "*" || op->field == "/" || op->field == ".*" || op->field == "./"))
    {
      ...
      else if (lhs->value.matrix().size() == 1)
      {
        ...
        if (op->field == "*" || op->field == ".*")
          ...
          lhs->value.local() = Derived::Constant(rhs->value.matrix().rows(), rhs->value.matrix().cols(), temp).array() /
                               rhs->value.matrix().array();
        lhs->value.mapLocal();
        lhs->type = VALUE;
      }
      else if ((op->field == ".*" || op->field == "./") && lhs->value.matrix().rows() == rhs->value.matrix().rows() &&
               lhs->value.matrix().cols() == rhs->value.matrix().cols())
      {
        if (lhs->value.isLocal())
        {
...
void Parser<Derived>::evalAssignment(ChunkArray &chunks)
{
  ...
  {
    if (op->type == OPERATOR && op->field == "=" && (lhs->type == VALUE || lhs->type == VARIABLE) &&
        (rhs->type == VALUE || rhs->type == VARIABLE))
    {
      if (rhs->type == VARIABLE)
      {
...
void Parser<Derived>::printVars(size_t maxRows, size_t maxCols, int precision)
{
  for (typename ValueMap::iterator it = mVariables.begin(); it != mVariables.end(); it++)
    std::cout << it->first << " (" << it->second.matrix().rows() << "x" << it->second.matrix().cols()
...
T Parser<Derived>::stringToNumber(const std::string &str)
{
  ...
  T number;
  iss >> number;
  if (iss.fail() || iss.bad() || !iss.eof()) throw std::runtime_error("Failed to convert " + str + " to a number.");
  return number;
}
...
#ifdef DEBUG
template <typename Derived>
void Parser<Derived>::test_w_lt(size_t &numFails, typename Derived::Scalar & /* s */, Derived &a34, Derived &b34,
                                Derived & /* c43 */, Derived & /* v */, std::true_type)
{
  ...
  if (resultMatrix.isApprox(resultValue.matrix()))
  ...
  resultMatrix.setConstant(1, 1,
                           std::abs(a34.maxCoeff()) >= std::abs(a34.minCoeff()) ? a34.maxCoeff() : a34.minCoeff());
  if (resultMatrix.isApprox(resultValue.matrix()))
    std::cout << "OK" << std::endl;
  else
  {
  ...
  {
    if (std::abs(resultMatrix(i)) < std::abs(temp(i))) resultMatrix(i) = temp(i);
  }
  if (resultMatrix.isApprox(resultValue.matrix()))
    std::cout << "OK" << std::endl;
  else
...
template <typename Derived>
void Parser<Derived>::test_w_lt(size_t & /* numFails */, typename Derived::Scalar & /* s */, Derived & /* a34 */,
                                Derived & /* b34 */, Derived & /* c43 */, Derived & /* v */, std::false_type)
{
  // do nothing
}
...
size_t Parser<Derived>::test()
{
  ...
  std::cout << "Make sure this function completes successfuly and prints the message "
               "'Successfully completed unit test for EigenLab with no failures.'"
            << std::endl;
  std::cout << std::endl;
  ...
  resultMatrix = a34;
  for (Index i = 0; i < a34.size(); ++i) resultMatrix(i) = pow(std::abs(a34(i)), b34(i));
  //		std::cout << std::endl;
  ...
  resultMatrix = a34;
  for (Index i = 0; i < a34.size(); ++i) resultMatrix(i) = pow(std::abs(a34(i)), s);
  if (resultMatrix.isApprox(resultValue.matrix()))
    std::cout << "OK" << std::endl;
  else
  {
  ...
  resultValue = eval("s ^ b");
  resultMatrix = b34;
  for (Index i = 0; i < b34.size(); ++i) resultMatrix(i) = pow(s, b34(i));
  if (resultMatrix.isApprox(resultValue.matrix()))
    std::cout << "OK" << std::endl;
  else
  {
  ...
  if (var("x").matrix().isApprox(resultValue.matrix()))
    std::cout << "OK" << std::endl;
  else
  {
    std::cout << "FAIL" << std::endl;
