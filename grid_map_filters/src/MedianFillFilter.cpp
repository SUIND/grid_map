                                     ...
                                     const size_t radiusInPixels, const grid_map::Size bufferSize)
{
  ...
  const auto& neighbourhood =
      inputMap.block(topLeftIndex(0), topLeftIndex(1), neighbourPatchSize(0), neighbourPatchSize(1));
  ...
  for (size_t row = 0; row < neighbourhood.rows(); ++row)
  {
    ...
    for (size_t col = 0; col < cols; ++col)
    {
      ...
      {  // Calculate the median of the finite neighbours.
  ...
  {  // Compute the median of the finite values in the neighbourhood.
...
bool MedianFillFilter<T>::update(const T& mapIn, T& mapOut)
{
  ...
  for (GridMapIterator iterator(mapOut); !iterator.isPastEnd(); ++iterator)
  {
    ...
    if (not std::isfinite(inputValue))
    {  // Fill the NaN input value with the median.
      outputValue = getMedian(inputMap, index, radiusInPixels, bufferSize);
      numNans++;
    }
    else if (filterExistingValues_)
    ...
    {  // Dont do any filtering, just take the input value.
