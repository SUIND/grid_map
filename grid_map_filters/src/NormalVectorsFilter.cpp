...
bool NormalVectorsFilter<T>::configure()
{
  ...
  if (algorithm != "raster")
  {
    // Read radius, if found, its value will be used for area method. If radius parameter is not found, raster method
    // will be used.
  ...
  // Read parallelization_enabled to decide whether parallelization has to be used, if parameter is not found an error
  // is thrown and the false default value will be used.
...
template <typename T>
void NormalVectorsFilter<T>::computeWithAreaSerial(GridMap& map, const std::string& inputLayer,
...
template <typename T>
void NormalVectorsFilter<T>::computeWithAreaParallel(GridMap& map, const std::string& inputLayer,
                                                     const std::string& outputLayersPrefix)
{
  const double start = ros::Time::now().toSec();
                                                         ...
                                                         const grid_map::Index& index)
{
  ...
  {
    ROS_WARN("Estimation radius is smaller than allowed by the map resolution (%f < %f)", estimationRadius_,
             minAllowedEstimationRadius);
  }
...
template <typename T>
void NormalVectorsFilter<T>::computeWithRasterSerial(GridMap& map, const std::string& inputLayer,
                                                     const std::string& outputLayersPrefix)
{
...
template <typename T>
void NormalVectorsFilter<T>::computeWithRasterParallel(GridMap& map, const std::string& inputLayer,
                                                       const std::string& outputLayersPrefix)
{
  const double start = ros::Time::now().toSec();
  ...
  if (submapBufferSize(1) != 0)
  {
    ...
    tbb::parallel_for(0, submapBufferSize(0) * submapBufferSize(1), [&](int range) {
      const Index index(range / submapBufferSize(1) + submapStartIndex(0),
                                                           ...
                                                           const grid_map::Index& index)
{
  ...
  const int configurationDirX = 1 * static_cast<int>(std::isfinite(topCell)) +
                                2 * static_cast<int>(std::isfinite(centralCell)) +
                                4 * static_cast<int>(std::isfinite(bottomCell));
  const int configurationDirY = 1 * static_cast<int>(std::isfinite(leftCell)) +
                                2 * static_cast<int>(std::isfinite(centralCell)) +
