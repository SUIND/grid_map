...
{
PointcloudProcessor::PointcloudProcessor() { params_ = std::make_unique<grid_map_pcl::PclLoaderParameters>(); }

void PointcloudProcessor::loadParameters(const std::string& filename) { params_->loadParameters(filename); }
...
Pointcloud::Ptr PointcloudProcessor::applyRigidBodyTransformation(Pointcloud::ConstPtr inputCloud) const
{
  auto transformedCloud = grid_map_pcl::transformCloud(
      inputCloud, grid_map_pcl::getRigidBodyTransform(params_->get().cloudTransformation_.translation_,
