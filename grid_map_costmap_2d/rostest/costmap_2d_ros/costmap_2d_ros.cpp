...
ROSCostmapServer::ROSCostmapServer(const std::string& name, const std::string& baseLinkTransformName,
                                   const grid_map::Position& origin, const double& width, const double& height)
#if ROS_VERSION_MINIMUM(1, 14, 0)
    : tfBuffer_(ros::Duration(1.0)), tfListener_(tfBuffer_)
#else
...
TransformBroadcaster::~TransformBroadcaster() { broadcastingThread_.join(); }

void TransformBroadcaster::shutdown() { shutdownFlag_ = true; }
...
bool fromCostmap2DROSAtRobotPose(costmap_2d::Costmap2DROS& ros_costmap, const grid_map::Length& geometry,
                                 const std::string& layer_name, grid_map::GridMap& grid_map)
{
  grid_map::Costmap2DConverter<grid_map::GridMap> converter;
  boost::lock_guard<costmap_2d::Costmap2D::mutex_t> lock(*(ros_costmap.getCostmap()->getMutex()));
...
TEST(Costmap2DROSConversion, full_window)
{
  ...
  ASSERT_EQ(grid_map_5x5.getLength().x(), ros_costmap_5x5.getROSCostmap()->getCostmap()->getSizeInCellsX() *
                                              ros_costmap_5x5.getROSCostmap()->getCostmap()->getResolution());
  ASSERT_EQ(grid_map_5x5.getLength().y(), ros_costmap_5x5.getROSCostmap()->getCostmap()->getSizeInCellsY() *
                                              ros_costmap_5x5.getROSCostmap()->getCostmap()->getResolution());
  ...
  std::cout << "Original cost: " << static_cast<int>(ros_costmap_5x5.getROSCostmap()->getCostmap()->getCost(8, 0))
            << std::endl;
...
TEST(Costmap2DROSConversion, cost_map_centres)
{
  ...
  std::cout << std::endl;
  std::string layer_name = "obstacle_costs";
  ROSCostmapServer ros_costmap_5x5_3x3_offset("five_by_five_three_by_three_offset", "base_link_5x5_3x3_offset",
                                              grid_map::Position(-6.0, 0.0), 5.0, 5.0);
  ROSCostmapServer ros_costmap_5x5_3x3_centre("five_by_five_three_by_three_centre", "base_link_5x5_3x3_centre",
                                              grid_map::Position(-6.0, -6.0), 5.0, 5.0);
  ROSCostmapServer ros_costmap_5x5_2_5x2_5_offset("five_by_five_twohalf_by_twohalf_offset",
                                                  "base_link_5x5_2_5x2_5_offset", grid_map::Position(-12.0, 0.0), 5.0,
                                                  5.0);
  ...
  fromCostmap2DROSAtRobotPose(*(ros_costmap_5x5_3x3_offset.getROSCostmap()), geometry_3x3, layer_name,
                              grid_map_5x5_3x3_offset);
  fromCostmap2DROSAtRobotPose(*(ros_costmap_5x5_3x3_centre.getROSCostmap()), geometry_3x3, layer_name,
                              grid_map_5x5_3x3_centre);
  grid_map::Length geometry_2_5x2_5(2.5, 2.5);
  fromCostmap2DROSAtRobotPose(*(ros_costmap_5x5_2_5x2_5_offset.getROSCostmap()), geometry_2_5x2_5, layer_name,
